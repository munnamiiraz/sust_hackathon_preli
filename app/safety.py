...
def contains_forbidden(text: str) -> list[str]:
  t = text.lower()
  return [vtype for pat, vtype in FORBIDDEN if re.search(pat, t)]

INJECTION = [
  r'ignore\s+(all\s+)?(previous|prior|above|your)?\s*(instructions?|rules?|prompts?|constraints?)',
  r'you\s+are\s+now\s+a', r'new\s+(instructions?|system\s+prompt)',
  r'\bdisregard\b', r'\bact\s+as\b', r'\bpretend\s+(you\s+are|to\s+be)\b',
  r'\bjailbreak\b', r'forget\s+(all\s+)?(previous|prior)?\s*(instructions?|rules?)',
  r'\bsystem\s*:\s*', r'\[system\]', r'<\s*system\s*>',
]

def is_prompt_injection(text: str) -> bool:
  t = text.lower()
  hit = any(re.search(p, t) for p in INJECTION)
  if hit: logger.warning("Prompt injection detected")
  return hit

REPLIES = {
  "wrong_transfer": "Thank you for reaching out. We have received your report regarding a transfer concern. Our team will investigate thoroughly. Any eligible amount will be processed through official channels. A specialist will personally review your case. For further assistance, please contact our official support channels.",
  "payment_failed": "Thank you for contacting us. We understand your concern about the payment issue. Our payments team will review the transaction details. Any eligible amount will be returned through official channels. For further assistance, please contact our official support channels.",
  "refund_request": "Thank you for your message. We have noted your request. Our team will review your case and any eligible amount will be returned through official channels. A specialist will personally review your case. For further assistance, please contact our official support channels.",
  "duplicate_payment": "Thank you for reporting this. We have recorded your concern regarding a possible duplicate charge. Our team will investigate and any eligible amount will be returned through official channels. For further assistance, please contact our official support channels.",
  "merchant_settlement_delay": "Thank you for contacting us. We have noted your merchant settlement concern. Our merchant operations team will review and follow up within the standard processing window. For further assistance, please contact our official support channels.",
  "agent_cash_in_issue": "Thank you for reaching out. We have received your cash-in report. Our agent operations team will investigate. Any eligible amount will be credited through official channels. For further assistance, please contact our official support channels.",
  "phishing_or_social_engineering": "Thank you for reporting this security concern. We take this very seriously. Please do not share any personal information with unknown contacts. Our security team has been alerted. For further assistance, please contact our official support channels.",
  "other": "Thank you for contacting us. A support specialist will review your case. For further assistance, please contact our official support channels.",
}
...
def apply_safety(response: TicketResponse, complaint: str, language: str = "en") -> TicketResponse:
  # Truncate at 5000 chars before regex — defence-in-depth against oversized input
  ...
  # Hard fallback if somehow our own template has violations (shouldn't happen)
