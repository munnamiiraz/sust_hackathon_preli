"""Pydantic v2 models for the Ticket Investigation API."""
...
_MAX_COMPLAINT_LEN = 5_000    # chars — prevents regex DoS on huge inputs
_MAX_TX_COUNT      = 100      # transactions per request
_MAX_ID_LEN        = 128      # ticket/transaction IDs
...
class TransactionEntry(BaseModel):
    """A single transaction record attached to a ticket."""
...
class TicketRequest(BaseModel):
    """Incoming ticket payload for analysis."""
...
class TicketResponse(BaseModel):
    """Outgoing analysis result for a ticket."""

    model_config = ConfigDict(extra="forbid")

    ticket_id: str
    # `None` serializes to JSON `null`. Never coerce to "".
